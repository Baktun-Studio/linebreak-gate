...
Schema (v1)::

    {
      ...
      "findings": [ {"cve_id","severity","cvss","epss","epss_percentile","kev",
                     "kev_date_added","kev_due_date","package","ecosystem",
                     "installed_version","fixed_version","advisory_url"} ],
      "risk_score": 0-100 | null,
      "remediation": { ...remediation_block()... } | null,
      "approvals": [ {"id","role","user_email","decision","notes","at"} ]
    }

The CI gate (``linebreak-gate scan``) additionally stamps, on ``cve_scan``
records, ``exploit_intel`` (the EPSS/KEV source status of that run, see
:mod:`linebreak_gate.exploit_intel`) and ``verdict``
(``{"passes", "block_reasons": ["kev" | "vulnerability"]}`` at scan time).
``epss`` is a probability in [0, 1] or null; ``kev`` is true (in CISA's
catalog), false (checked, not listed) or null (not checked / no CVE id).
"""
...
SCHEMA_VERSION = 1
KINDS = ("cve_scan", "code_scan", "remediation", "criteria_check")
...
def append_approval(
    ...
    *,
    ...
    ``identity`` (optional) is the verified-identity block the CI gate records
    next to ``identity_source`` (provider, email, login); ``declared_by`` is
    the typed ``--approver`` when it differed from the verified subject. Both
    are omitted from the entry when absent, so existing call sites are unchanged.
    ...
    ``expires`` (optional, ``YYYY-MM-DD``) bounds an override in time: past
    that date the accepted risk blocks again (``expired_risk``). A renewal is a
    NEW entry with a later ``at``; earlier entries stay in the trail as
    ...
    if identity_source is not None:
        # LIN-41: account | git | local-placeholder — whether user_email
        # names a real person an auditor can map.
        entry["identity_source"] = identity_source
    if identity:
...
def update_approval(
    ...
    *,
    ...
    """Merge ``patch`` into the approval entry ``approval_id`` (e.g. the ticket
    key the tracker assigned after the record was written, or the error that
    kept it from being assigned). Keys mapped to ``None`` are removed. Returns
    the updated entry, or None when the artifact or the entry does not exist.
    Only annotation keys are meant to travel here: the decision, approver,
    reason and timestamp of a recorded approval are never rewritten."""
    doc = read_artifact(project_root, name, base_dir=base_dir)
    if doc.get("kind") not in KINDS:
        return None
        ...
        if isinstance(entry, dict) and entry.get("id") == approval_id:
            for key, value in patch.items():
                if value is None:
                    ...
                    entry[key] = value
            write_artifact(project_root, name, doc, base_dir=base_dir)
            return entry
    return None
