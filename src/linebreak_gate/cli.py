...
def _cmd_check(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        return _emit_signature_block(str(e), args.format)
...
def _emit_signature_block(message: str, fmt: str) -> int:
    ...
    if fmt == "json":
        print(
            json.dumps(
                {"passes": False, "status": "signature-invalid", "message": message}, indent=2
            )
        )
    else:
        print(
...
def _emit_check(payload: dict[str, Any], fmt: str) -> None:
    ...
    if b.get("signature") == "verified":
        print(f"  signed approval VERIFIED (key {b.get('signing_kid')}, by {b.get('signed_by')})")
    else:
        print(
