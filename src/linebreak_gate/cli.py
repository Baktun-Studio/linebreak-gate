...
from .criteria_check import MANUAL_POLICIES, PHASES, STAGES
...
def _actor() -> str:
    """The acting user for audit records, read through the one CI-environment
    reader (GitHub, GitLab, Bitbucket, Azure), then the shell's user."""
    return ci_env.actor()
...
def _tickets_config(cfg: Any) -> Any:
    """The tracker in force: the ``tickets:`` block of gate.yml when the
    repository fixes one, else what the governance service's Integrations
    screen holds (read with the pipeline token). Reading it can never change
    a verdict: any trouble is a warning and no tracker."""
    if cfg.tickets is not None:
        return cfg.tickets
    try:
        return tickets.from_governance()
    except Exception as e:  # noqa: BLE001 - the tracker never blocks
        _err(
            "tickets: could not read the tracker configuration from the governance service "
            f"({e}); the verdict is unaffected"
        )
        return None
...
def _cmd_scan(args: argparse.Namespace) -> int:
    ...
    tickets_cfg = _tickets_config(cfg)
    if tickets_cfg is not None:
        ...
        _tickets_hook(tickets.on_scan, root, tickets_cfg, current=current, prior_ids=prior_ids)
    return 0 if payload["passes"] else 1


def _cmd_report_governance(args: argparse.Namespace) -> int:
    """``report --from-governance``: the last run PUBLISHED to the governance
    service, read with the governance token. Read only. Exit 0 when shown (or
    when nothing was published yet), 2 when it could not be read."""
    from . import governance_report as gr

    try:
        report = gr.fetch_report(
            governance_env.resolve(),
            project=args.project,
            run_id=args.run_id,
            stage=args.stage,
        )
    except gr.GovernanceReportError as e:
        _err(str(e))
        return 2
    if report is None:
        stage = f" at stage {args.stage}" if args.stage else ""
        message = (
            f"No run published{stage} to the governance service for this project yet "
            "(CI publishes with `linebreak-gate publish`)."
        )
        if args.format == "json":
            print(json.dumps({"passes": None, "error": "no published run"}, indent=2))
        else:
            print(message)
        return 0
    if args.format == "json":
        ...
        print("\n".join(gr.render(report)))
    return 0


def _cmd_report(args: argparse.Namespace) -> int:
    if getattr(args, "from_governance", False):
        return _cmd_report_governance(args)
    if args.project or args.run_id or args.stage:
        _err("--project, --run-id and --stage apply only with --from-governance")
        return 2
    root = Path(args.path).resolve()
    cfg = resolve_config(root, cli_fail_on=args.fail_on)
...
def _cmd_override(args: argparse.Namespace) -> int:
    ...
    ticket = _tickets_hook(
        tickets.on_acceptance, root, _tickets_config(cfg), artifact=name, entry=entry
    )
...
def _check_label(check: dict[str, Any]) -> str:
    ...
    if check.get("resource"):
        label += f", resource: {check['resource']}"
    env = check.get("environment")
    if isinstance(env, dict):
        label += f", environment: {env.get('name') or env.get('version_url')}"
    expect = check.get("expect")
    if isinstance(expect, dict):
        wanted = expect.get("output")
        wanted = wanted if isinstance(wanted, list) else ([wanted] if wanted else [])
        parts = [f"exit {expect['exit']}"] if expect.get("exit") not in (None, 0) else []
        parts += [repr(w) for w in wanted]
        label += f", expect: {' and '.join(parts) or 'exit 0'}"
    return label


def _tests_run_label(r: dict[str, Any]) -> str:
    """``, 12 tests`` next to a criterion whose runner reported a count
    (issue #259): a ``(0 tests)`` in the report is visible, a bare pass is
    not. A ``tests`` criterion whose runner reported nothing says so."""
    if "tests_run" not in r:
        return ""
    n = r["tests_run"]
    if n is None:
        return ", test count unknown"
    return f", {n} test" + ("" if n == 1 else "s")
...
def _cmd_check(args: argparse.Namespace) -> int:
    ...
    root = Path(args.path).resolve()
    if args.phase != "verify" and args.stage != "release":
        _err("--phase prepare applies only to --stage release (gate stays closed)")
        return 2
    if args.stories == ["auto"]:
        _resolve_auto_story(args, root)
    elif args.stories and "auto" in args.stories:
        _err("--story auto cannot be combined with other --story ids (gate stays closed)")
        return 2
    ...
    # Sign-offs recorded on the governance service (the panel) count like the
    # repository's own, under the same rules. A service that does not answer
    # neither blocks nor approves: the notice says so and the check goes on.
    governance_records, notice = signoffs.load_governance_signoffs()
    if notice:
        _err(notice)
    try:
        payload = criteria_check.evaluate_bundle(
            ...
            stage=args.stage,
            extra_signoffs=governance_records,
            phase=args.phase,
            integrity_policy=cfg.criteria_integrity,
        )
...
def _resolve_auto_story(args: argparse.Namespace, root: Path) -> None:
    """``--story auto`` (issue #256), the same rule as ``ci --story auto`` and
    the Action: the approved story in a ``feat/<id>`` or ``story/<id>`` branch,
    else the started stories only. Says which on stderr so the log shows it."""
    from . import ci_cmd

    branch = ci_env.detect().branch or _git_branch(root)
    inferred = ci_cmd.infer_story(root, branch)
    if inferred:
        args.stories = [inferred]
        _err(f"--story auto: story {inferred} inferred from branch {branch}")
    else:
        args.stories = None
        args.started_only = True
        _err(
            f"--story auto: no approved story id in branch '{branch or ''}'; "
            "evaluating started stories only"
        )
...
def _cmd_tickets_sync(args: argparse.Namespace) -> int:
    ...
    root = Path(args.path).resolve()
    cfg = resolve_config(root)
    completed, remaining, messages = tickets.sync(root, _tickets_config(cfg))
    for m in messages:
        print(f"  {m}")
...
def _emit_check(payload: dict[str, Any], fmt: str) -> None:
    ...
    for r in payload["criteria"]:
        ...
        label = _one_line(_check_label(r["check"]) + _tests_run_label(r))
        print(
        ...
        if env:
            print(f"      {_environment_line(env)}")
        for f in r.get("integrity") or []:
            word = "BLOCKING" if f["policy"] == "block" else "warning"
            print(f"      integrity {word} ({f['kind']}): {_one_line(f['detail'])[:400]}")
        if r["result"] in ("fail", "error", "collision") and r.get("detail"):
            # The integrity lines above already say why; lead with the run.
            lines = [
                ln for ln in r["detail"].strip().splitlines() if not ln.startswith("integrity (")
            ]
            if lines:
                print(f"      {_one_line(lines[0])[:200]}")
    ...
    # One line per attestation criterion the prepare phase left for the
    # sign-off made on this run ("awaiting attestation: <id> (<story>)").
    for a in payload.get("awaiting_attestation") or []:
        print(
            f"  awaiting attestation: {a['id']} ({a['story']}) (sign it off against this run, "
            "then run --phase verify)"
        )
    # A red that turned out to be a clash over a shared resource (#262).
    for c in payload.get("collisions") or []:
        print(
            f"  collision: {c['id']} ({c['story']}) on resource {c['resource']}: failed while "
            "the resource was in use, passed when re-run alone (not a defect, not blocking)"
        )
    # What an environment-bound check measured, when not the evaluated commit.
    for w in payload.get("environment_warnings") or []:
        print(f"  environment warning: {_one_line(w['message'])}")
    ...
    integrity_warnings = _print_integrity_summary(payload)
    ...
    if payload["tool_error"]:
        ...
        if integrity_warnings:
            tail += (
                f"; {integrity_warnings} integrity warning(s) (not blocking; criteria.integrity: "
                f"block in {Path('.linebreak') / 'gate.yml'} makes them blocking)"
            )
        ...
        awaiting = len(payload.get("awaiting_attestation") or [])
        if awaiting:
            tail += (
                f"; prepare phase: {awaiting} attestation criterion(s) not evaluated. This run "
                "is the one to attest, NOT the release: sign them off against it, then run "
                "--phase verify"
            )
        head = (
...
def _environment_line(env: dict[str, Any]) -> str:
    """``measured <name> at <version> (<status>...)`` for one criterion."""
    version = env.get("version") or "unknown version"
    status = env.get("status")
    extra = ""
    if status == "behind" and env.get("behind") is not None:
        extra = f", {env['behind']} commit(s) behind the evaluated commit"
    elif status == "unreachable" and env.get("note"):
        extra = f": {env['note']}"
    elif status == "changed":
        extra = f", then {env.get('version_after')}"
    if env.get("behind_main"):
        extra += f", {env['behind_main']} behind {env.get('main')}"
    return _one_line(f"measured {env.get('name')} at {version} ({status}{extra})")


def _print_integrity_summary(payload: dict[str, Any]) -> int:
    """One line with the integrity totals and the policy in force, plus one
    per kind that could not be looked at (never read as "all clear").
    Returns the number of non-blocking warnings."""
    block = payload.get("integrity") or {}
    findings = block.get("findings") or []
    warnings = sum(1 for f in findings if f.get("policy") == "warn")
    blocking = sum(1 for f in findings if f.get("policy") == "block")
    if findings:
        policy = ", ".join(f"{k}={v}" for k, v in (block.get("policy") or {}).items())
        print(
            f"  integrity: {warnings} warning(s), {blocking} blocking "
            f"(criteria.integrity: {policy})"
        )
    for kind, why in sorted((block.get("skipped") or {}).items()):
        print(f"  integrity: {kind} not checked ({why})")
    return warnings
...
def _cmd_override_criterion(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        return 2
    ticket = _tickets_hook(
        tickets.on_acceptance, root, _tickets_config(cfg), artifact="criteria", entry=entry
    )
...
def _cmd_publish(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if args.dry_run:
            print(json.dumps(payload, indent=2))
            return 0
        # One lookup for the service credentials (governance_env): variables
        # first, then ~/.config/linebreak/, then ~/.linebreak/env. The pipeline
        # token LINEBREAK_GOV_TOKEN wins over LINEBREAK_GOVERNANCE_TOKEN here,
        # as before, because it is the one CI maps for publishing.
        creds = governance_env.resolve()
        token = os.environ.get(governance_env.PIPELINE_TOKEN, "").strip() or creds.token or ""
        project = (args.project or creds.project or "").strip()
        url = (args.to or creds.base_url or "").strip()
        if not token:
            raise publish.PublishError(
                "no governance token (LINEBREAK_GOV_TOKEN, LINEBREAK_GOVERNANCE_TOKEN or "
                "~/.config/linebreak/governance.env)"
            )
        if not project:
            raise publish.PublishError("no project id (--project or LINEBREAK_GOV_PROJECT)")
        if not url:
            raise publish.PublishError("no service URL (--to or LINEBREAK_GOVERNANCE_BASE_URL)")
        publish.send(payload, url=url, project_id=project, token=token)
    ...
    print(
        ...
        f"to {url.rstrip('/')} for project {project}"
    )
    return 0
...
def _print_approval_summary(root: Path, rel: str, stories: list[dict[str, Any]]) -> None:
    """What is about to be approved, and what changes against the approval in
    force (issues #258 and #261): the approver sees the count and every
    reworded statement before and after, never just "approved"."""
    from . import spec_bundle, spec_integrity

    print(
        f"About to approve {rel}: {len(stories)} story(ies), "
        f"{spec_integrity.count_criteria(stories)} criterion(s)."
    )
    try:
        prior = spec_bundle.load_bundle(root)
    except spec_bundle.SpecBundleError as e:
        print(
            f"The approved bundle in {spec_bundle.SPEC_DIR.as_posix()}/ is unreadable ({e}): no comparison."
        )
        return
    if prior is None:
        print(f"No approved bundle yet in {spec_bundle.SPEC_DIR.as_posix()}/: every story is new.")
        return
    old = prior["stories"]
    diff = spec_integrity.diff_stories(old, stories)
    head = (
        f"Compared with the approval in force ({len(old)} story(ies), "
        f"{spec_integrity.count_criteria(old)} criterion(s))"
    )
    if spec_integrity.diff_is_empty(diff):
        print(f"{head}: no story or criterion changes.")
        return
    print(f"{head}:")
    for line in spec_integrity.format_diff(diff):
        print(line)


def _refuse_unconfirmed_draft(fresh: Any, rel: str, stories: list[dict[str, Any]]) -> int:
    """Refuse to approve a draft that is not confirmed to be the one on the
    remote (issue #258): a valid signature over the wrong document is worse
    than no signature. Says what differs and what to do."""
    from . import spec_integrity

    # The summary went to stdout: keep it ahead of the refusal when both
    # streams land in the same log.
    sys.stdout.flush()
    reasons = {
        "stale": f"the draft on disk is NOT the one on {fresh.ref}",
        "missing": f"the draft is not on {fresh.ref}",
        "unreachable": f"could not confirm the draft against {fresh.ref}",
        "outside": "the draft is outside the repository, so it cannot be confirmed",
    }
    _err(f"{reasons.get(fresh.status, fresh.status)}; nothing approved ({fresh.detail}).")
    if fresh.status == "stale" and fresh.remote_stories is not None:
        remote = fresh.remote_stories
        print(
            f"  {fresh.ref} has {len(remote)} story(ies), "
            f"{spec_integrity.count_criteria(remote)} criterion(s); the copy on disk has "
            f"{len(stories)} story(ies), {spec_integrity.count_criteria(stories)} criterion(s).",
            file=sys.stderr,
        )
        print(f"  On {fresh.ref} (+) and only on disk (-):", file=sys.stderr)
        for line in spec_integrity.format_diff(spec_integrity.diff_stories(stories, remote)):
            print(line, file=sys.stderr)
    hint = {
        "stale": "Update the checkout (git pull) and approve again",
        "missing": "Push the draft so the approval covers what was reviewed",
        "unreachable": "Check the connection to the remote and approve again",
        "outside": "Move the draft into the repository",
    }.get(fresh.status, "Fix the checkout")
    print(
        f"  {hint}, or pass --local to approve the copy on disk knowingly "
        f"(linebreak-gate spec approve {rel} --local ...).",
        file=sys.stderr,
    )
    return 1


def _print_approval_integrity(root: Path, stories: list[dict[str, Any]]) -> None:
    """Integrity warnings on the draft itself, before it is signed: stories
    that run each other's tests (#263) and statements naming identifiers gone
    from the code (#261). Never blocking here: ``check`` applies the
    ``criteria.integrity`` policy."""
    from . import spec_integrity

    try:
        policy = resolve_config(root).criteria_integrity
    except GateConfigError:
        from .gate_config import IntegrityPolicy

        policy = IntegrityPolicy()
    findings: list[dict[str, Any]] = []
    if policy.of("shared_tests") != "off":
        findings += spec_integrity.shared_tests(stories)
    if policy.of("stale_statements") != "off":
        findings += spec_integrity.stale_statements(root, stories).findings
    if not findings:
        return
    print(
        f"Integrity warnings on this draft ({len(findings)}; `check` applies criteria.integrity):"
    )
    for f in findings:
        print(f"  {f['kind']}: {f['id']} ({f['story']}): {_one_line(f['detail'])}")
...
def _cmd_spec_approve(args: argparse.Namespace) -> int:
    ...
    failure (unwritable tree) or a draft that is not the one on the remote,
    2 = invalid draft (fail closed on structure).

    Before writing anything it prints what is about to be approved (stories,
    criteria, and against the current approval every story and criterion
    added, removed or reworded, with the statement before and after), and it
    fetches the draft's version on the remote: a copy on disk that differs
    from it, is not on it, or cannot be compared is REFUSED unless
    ``--local`` says the local copy is deliberate (issue #258). A repository
    without a remote has no copy that could be ahead, so it proceeds with a
    notice.
    """
    ...
    from . import spec_bundle, spec_integrity, spec_write
    ...
    rel = _rel_or_abs(draft_path, root)

    _print_approval_summary(root, rel, stories)
    fresh = spec_integrity.draft_freshness(root, draft_path, data, against=args.against)
    if fresh.status == "fresh":
        print(f"The draft matches {fresh.ref} (fetched just now).")
    elif fresh.status == "no-remote":
        print(
            f"No remote to compare the draft against ({fresh.detail}): approving the copy on disk."
        )
    elif args.local:
        print(
            f"WARNING: --local: approving the copy on disk without confirming it against "
            f"{fresh.ref or 'the remote'} ({fresh.detail})."
        )
    else:
        return _refuse_unconfirmed_draft(fresh, rel, stories)
    _print_approval_integrity(root, stories)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    _add_common(report)
    # The scan runs in CI; this reads what CI published, outside CI.
    report.add_argument(
        "--from-governance",
        action="store_true",
        help="read the last run published to the governance service instead of the scan "
        "recorded here (read only; credentials from the environment or "
        "~/.config/linebreak/governance.env)",
    )
    report.add_argument(
        "--project",
        default=None,
        help="with --from-governance: project id on the service (or LINEBREAK_GOV_PROJECT)",
    )
    report.add_argument(
        "--run-id",
        default=None,
        help="with --from-governance: a specific published run instead of the last one",
    )
    report.add_argument(
        "--stage",
        choices=STAGES,
        default=None,
        help="with --from-governance: the last run of this stage (default: any stage)",
    )
    ...
    scope.add_argument(
        ...
        "`auto`: the story in a feat/<id> or story/<id> branch, else started stories only",
    )
    scope.add_argument(
        "--started-only",
        action="store_true",
    ...
    check.add_argument(
        ...
        choices=PHASES,
        default="verify",
        help="release phase: verify (default) enforces every criterion; prepare is the run "
        "an attestation is made on: criteria marked `check.when: attestation` are not "
        "evaluated (their sign-off attests this run), everything else is enforced",
    )
    ...
    from . import init_cmd as _init_cmd

    init.add_argument(
        "--provider",
        choices=("auto", *_init_cmd.PROVIDERS, "all"),
        default="auto",
        help="which pipeline file(s) to write: github (.github/workflows/security-gate.yml), "
        "bitbucket (bitbucket-pipelines.yml), azure (azure-pipelines.yml), all, or auto "
        "(from the git remote; GitHub when unknown)",
    )

    # The whole run for CI providers without a native Action: scan + scoped
    # check + evidence directory + PR comment and build status through the
    # provider's API (Bitbucket Pipelines, Azure DevOps), exit with the
    # worse code.
    ci = sub.add_parser(
        "ci",
        help="run scan + check, write the evidence, post the PR comment and build status "
        "(Bitbucket Pipelines, Azure DevOps), and exit 0/1/2",
    )
    ci.add_argument("--path", default=".", help="project root (default: .)")
    ci.add_argument(
        "--fail-on",
        choices=FAIL_ON_LEVELS,
        default=None,
    ...
    ci.add_argument(
        ...
        default="all",
        metavar="all|auto|ID",
        help="check scope: all (every approved story, default), auto (the story in a "
        "feat/<id> or story/<id> branch, else started stories only), or a story id",
    )
    ci.add_argument(
        "--manual",
        choices=("auto", *MANUAL_POLICIES),
        default="auto",
        help="manual criteria without a sign-off: warn or block; auto (default) is warn on "
        "a pull request and block elsewhere",
    )
    ci.add_argument(
        "--stage",
        choices=("auto", *STAGES),
        default="auto",
        help="check stage: pr or release; auto (default) is pr on a pull request and "
        "release elsewhere",
    )
    ci.add_argument(
        "--phase",
        choices=PHASES,
        default="verify",
        help="release phase: verify (default) or prepare (the run an attestation is made on; "
        "criteria marked `check.when: attestation` are not evaluated)",
    )
    ci.add_argument(
        "--out-dir",
        default=None,
        help="where to leave report.txt, criteria.txt, report.json, comment.md and the audit "
        "records for the pipeline artifact (default: .linebreak/ci-out)",
    )
    ci.add_argument("--no-comment", action="store_true", help="do not post the PR summary comment")
    ci.add_argument("--no-status", action="store_true", help="do not set the build status")
    ...
    spec_approve.add_argument(
        ...
        action="store_true",
        help=(
            "approve the copy on disk even when it cannot be confirmed to match the remote "
            "(no network, draft not pushed, or deliberately local)"
        ),
    )
    spec_approve.add_argument(
        "--against",
        ...
        default=None,
        help=(
            "remote branch the draft must match (default: the current branch's upstream, "
    ...
    publish.add_argument(
        "--to",
        default=None,
        help="governance service base URL (default: LINEBREAK_GOVERNANCE_BASE_URL, then "
        "the credential files; see README)",
    )
    publish.add_argument(
