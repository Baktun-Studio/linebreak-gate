...
from .criteria_check import MANUAL_POLICIES
...
def _cmd_override(args: argparse.Namespace) -> int:
    ...
    sa.append_approval(
        ...
        notes=reason,
        finding=record,
        # LIN-41: --approver is human-typed but unverified — marked "client"
        # so auditors can distinguish it from a session-backed identity.
...
def _emit_check(payload: dict[str, Any], fmt: str) -> None:
    ...
    for r in payload["criteria"]:
        ...
        print(
            f"  {icon} [{r['result']}] {r['story']}/{r['id']}  ({_one_line(spec)})  "
            f"{_one_line(r['statement'])}"
        )
        if r.get("signoff"):
            s = r["signoff"]
            print(
                f"      signed off by {_one_line(s['approver'])} at {s['signed_at']}: "
                f"{_one_line(s['note'])}"
            )
        if r.get("override"):
            o = r["override"]
            print(f"      overridden by {_one_line(o['approver'])}: {_one_line(o['reason'])}")
    ...
    if payload["tool_error"]:
        ...
        print(f"VERDICT: PASS. All criteria in scope satisfied ({summary}){tail}.")
    else:
        print(
...
def _print_scope(scope: dict[str, Any]) -> None:
    ...
    print(
        ...
        f"manual criteria: {scope['manual']}"
    )
...
def _cmd_spec_check(args: argparse.Namespace) -> int:
    ...
    out = bridge.check_story(Path(args.path).resolve(), args.story_id)
    if not out["ok"]:
        return _bridge_fail(out)
    icons = _result_icons()
