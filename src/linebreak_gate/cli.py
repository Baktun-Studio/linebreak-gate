...
from . import code_scan, entitlements, exploit_intel, llm, security_scan
...
def _override_ids(
    doc: dict[str, Any], detector: str = "dep", policy: _roles.RolesPolicy = _roles.EMPTY_POLICY
) -> tuple[set[str], list[dict[str, Any]]]:
    """The finding ids acknowledged by a recorded override that the roles in
    force still accept, plus one denial record per override they reject (the
    finding stays blocking; the entry stays on disk as audit)."""
...
def _detector_payload(
    ...
    override_ids, denials = _override_ids(doc, detector, policy)
    verdict = evaluate(
        findings,
        fail_on=cfg.fail_on,
...
def _evaluate_all(
    ...
    *,
    ...
    dependencies = _detector_payload(sec_doc, "dep", cfg, policy)
    code = _detector_payload(code_doc, "code", cfg, policy)
...
def _emit_check(payload: dict[str, Any], fmt: str) -> None:
    ...
    for r in payload["criteria"]:
        ...
        if r.get("override"):
            o = r["override"]
            print(
                f"      overridden by {_one_line(o['approver'])}{_role_note(o)}: "
    ...
    if payload["tool_error"]:
        ...
        print(
            ...
            f"(linebreak-gate override --criterion <id> ...).{tail}"
        )
...
def _cmd_override_criterion(args: argparse.Namespace) -> int:
    ...
    last = (entry.get("approvals") or [{}])[-1]
    note = _role_note(
        {
            "role": last.get("role") if last.get("role") != "approver" else None,
            "identity_source": ident.source,
        }
    )
    print(
