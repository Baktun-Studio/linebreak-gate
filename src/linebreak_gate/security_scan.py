...
critical|high|medium|low (lowercased) or "unknown". The CLI's ``scan`` then
stamps the exploitation fields (``epss``, ``kev``, ...) via
:mod:`linebreak_gate.exploit_intel`; :func:`finding_risk` reads them.
...
def _epss_value(finding: dict[str, Any]) -> float | None:
    try:
        score = float(finding.get("epss"))
    except (TypeError, ValueError):
        return None
    return score if 0.0 <= score <= 1.0 else None


def finding_risk(finding: dict[str, Any]) -> int:
    """A 0-100 risk score for ONE finding, exploitation first.

    * In CISA's KEV catalog (exploited in the wild today): 100, whatever the
      severity says.
    * Otherwise the severity weight (critical 100, high 80, medium 50, low 20)
      raised to the EPSS probability in percent when that is higher, so a
      "medium" with EPSS 0.93 scores 93.

    Exploitation data only ever RAISES the score: a finding with no EPSS/KEV
    data (offline scan, no CVE id) scores exactly as before this data existed,
    so the desktop's in-app gate and older artifacts keep their numbers.
    """
    ...
    if epss is not None:
        weight = max(weight, round(epss * 100))
    return weight


def compute_risk_score(findings: list[dict[str, Any]]) -> int | None:
    """A 0-100 risk score = the worst finding's :func:`finding_risk`; None if
    clean. Deterministic: same findings, same score."""
    if not findings:
        return None
    return max(finding_risk(f) for f in findings)
