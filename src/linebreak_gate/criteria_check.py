...
def _select_override(
    ...
    """:func:`_matching_override` under the roles in force: the first
    hash-matching override the current policy accepts, plus a denial record per
    ...
    for entry in overrides:
        finding = entry["finding"]
        if finding["criterion_id"] != criterion["id"] or finding.get("criterion_hash") != want:
            continue
...
def record_criterion_override(
    ...
    *,
    ...
    as in :func:`signoffs.record_signoff`."""
...
def _evaluate_criterion(
    ...
    if entry["result"] == "role-denied":
        ...
        entry["detail"] = entry["denials"][-1]["detail"] + (
            f"\n{cached.detail}" if cached.detail else ""
        )
    elif cached.detail and entry["result"] != "overridden":
...
def _write_results_artifact(root: Path, payload: dict[str, Any], *, actor: str | None) -> None:
    ...
    doc = sa.new_artifact(
        ...
        findings=[
            {
                k: r.get(k)
                for k in ("id", "story", "statement", "check", "result", "detail", "denials")
                if k != "denials" or r.get("denials")
            }
            for r in payload["criteria"]
        ],
