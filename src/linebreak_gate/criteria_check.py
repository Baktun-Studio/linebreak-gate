...
def evaluate_bundle(
    ...
    *,
    ...
    ``story_ids`` narrows the evaluation to those stories' criteria (LIN-55:
    the bridge's per-story check) — the other stories' checks are never run. A
    narrowed evaluation NEVER writes the results artifact: a partial result
    recorded as the project's criteria verdict would read as a full pass.
