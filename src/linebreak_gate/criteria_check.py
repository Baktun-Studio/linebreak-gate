...
def evaluate_bundle(
    ...
    *,
    ...
    overridden | error`` plus attribution for sign-offs and overrides.
    """
    ...
    scope["criteria_evaluated"] = len(results)
...
def _write_results_artifact(root: Path, payload: dict[str, Any], *, actor: str | None) -> None:
    ...
    scope = payload["scope"]
    if scope["mode"] != "all" or scope["manual"] != "block":
        summary = (
            f"{summary} (scope: {scope['mode']}, manual: {scope['manual']}; not a full verdict)"
        )
    doc = sa.new_artifact(
        "criteria_check",
        id="criteria",
