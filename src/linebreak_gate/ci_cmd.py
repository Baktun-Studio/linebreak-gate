...
def resolve_check_scope(
    ...
    *,
    ...
    if phase not in ("verify", "prepare") or (phase == "prepare" and stage != "release"):
        return (
            None,
            f"invalid 'phase' input '{phase}' with stage '{stage}' (expected verify, or "
            "prepare with stage release); gate stays closed",
            manual,
            stage,
        )
...
def run_ci(
    *,
    ...
    flags, note, manual, stage = resolve_check_scope(
        root, env=env, story=story, manual=manual, stage=stage, phase=phase
    )
    criteria_text = f"linebreak-gate ci: {note}\n"
    if flags is not None and phase == "prepare":
        criteria_text += (
            "linebreak-gate ci: release phase: prepare (the run to attest, not the release; "
            "run phase verify after the sign-off)\n"
        )
    if flags is None:
        check_code = 2
    else:
