...
#: An accepted risk whose acceptance ran out: it blocks again until renewed
#: or fixed (see :mod:`risk_acceptance`).
BLOCK_REASON_EXPIRED = "expired_risk"
BLOCK_REASONS = (BLOCK_REASON_KEV, BLOCK_REASON_VULNERABILITY, BLOCK_REASON_EXPIRED)
...
def evaluate(
    ...
    *,
    ...
    detector: str = "dep",
    expired_ids: set[str] | frozenset[str] = frozenset(),
    block_kev: bool = False,
    epss_threshold: float | None = None,
) -> dict[str, Any]:
    """Classify ``findings`` against the policy and the recorded overrides.

    Returns ``{"passes", "findings", "blocking", "acknowledged", "expired",
    "block_reasons"}`` where ``findings`` is the input annotated once with
    ``id`` (the override tuple), ``status`` (``blocking`` | ``acknowledged`` |
    ``expired_risk`` | ``below_floor``), ``risk`` (:func:`finding_risk`),
    ``exploitation`` (the label), ``triggers`` and ``block_reason``;
    ``blocking``/``acknowledged``/``expired`` are views of the same annotated
    dicts, so the classification the gate enforces and the one reports render
    can never diverge. All lists are in priority order (KEV, EPSS, severity).
    An acknowledged finding is one whose exact tuple id appears in
    ``override_ids`` (recorded human overrides still in force). A finding in
    ``expired_ids`` had an acceptance that has run out: it is ``expired_risk``,
    its ``block_reason`` is ``expired_risk`` (its ``triggers`` still say what
    made it block in the first place) and it BLOCKS again (listed under
    ``blocking`` too) until renewed or fixed. ``block_reasons`` lists the
    distinct reasons of the BLOCKING findings in :data:`BLOCK_REASONS` order.
    """
    ...
    for f in sort_by_priority(list(findings or [])):
        ...
        if reason is None:
            status = "below_floor"
        elif fid in expired_ids:
            status = "expired_risk"
            reason = BLOCK_REASON_EXPIRED
