...
def build_server(project_root: Path | str):
    ...
    def get_story(story_id: str) -> dict[str, Any]:
        ...
        command | manual; optional payload; optional when: release, meaning
        the criterion is checked only by the release gate, not on PRs). Read
        this BEFORE implementing the story: the criteria are the approved
        definition of done."""
    ...
    @server.tool()
    def check_story(story_id: str, stage: str = "release") -> dict[str, Any]:
        ...
        pass | fail | needs-signoff | overridden | error per criterion, or
        release-only for a `when: release` criterion when stage is "pr" (not
        evaluated; the release gate owns it). Use stage "pr" before pushing a
        pull request so checks against a shared staging environment do not
        run from your machine; "release" (default) evaluates everything."""
        return bridge.check_story(root, story_id, stage=stage)
