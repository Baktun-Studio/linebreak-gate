...
def check_story(
    ...
    *,
    ...
    ``stage="pr"`` skips ``check.when: release`` criteria (reported as
    ``release-only``, not evaluated), so a local check before a PR never runs
    scripts against a shared staging environment; ``"release"`` (default)
    evaluates everything, as the release gate does."""
    bundle, err = _load(project_root)
    if err:
        return err
    if _find(bundle, story_id) is None:
        return _unknown_story(bundle, story_id)
    kwargs: dict[str, Any] = {"story_ids": {story_id}, "write_artifact": False, "stage": stage}
    if run is not None:
        kwargs["run"] = run
