...
class ScanAbort(Exception):
    """Raised by ``discover``/``verify`` when the whole scan must stop — e.g.
    hosted credits exhausted, where every remaining verifier call would fail
    the same way. Unlike an ordinary verifier error (conservative: keep the
    finding), an abort fails the scan closed WITH its message, so the user
    sees the real cause instead of a wall of unverified findings."""
    ...
    **Fail closed**: if discovery raises, return an error result with NO findings
    ...
    for finding in parse_findings(raw):
        try:
            ...
            # A scan-wide failure (e.g. out of credits mid-verification): every
            # remaining call would fail identically, so keeping the findings
            # would report unverified noise while hiding the actual problem.
            return {
                "findings": [],
                "risk_score": None,
                "scanner": None,
