...
Integrity of the specification (epic e9, issues #258, #259, #260, #261, #263,
all found while building Katun): the gate checked that criteria pass; it now
also looks at whether they tell the truth. Existing specifications keep
working unchanged: the new findings warn by default, and only a zero-test
`tests` criterion and an unobserved `expect` fail on their own.

- `spec approve` (#258) prints, before writing anything, how many stories and
  criteria it is about to approve and, against the approval in force, every
  story and criterion added or removed and every statement or check that
  changed, before and after (#261). It then fetches the draft from the
  remote (the branch's upstream, else the remote's default branch, or
  `--against <remote>/<branch>`) and refuses (exit 1, nothing written) when
  the copy on disk differs, is not on the remote, or cannot be compared.
  `--local` approves the copy on disk knowingly. A repository without a
  remote proceeds with a notice.
- Zero tests executed (#259): the gate reads the runner's summary (pytest,
  vitest, jest, `node --test`, cargo, go) and records `tests_run` per
  criterion, shown in the report (`(tests: x, 0 tests)`). A `tests` criterion
  that executed zero tests FAILS even when the runner exited 0. A `command`
  whose runner executed zero tests is an integrity finding (`zero_tests`).
- `check.expect` (#260): a criterion can declare what it expects to observe,
  `output` (a string or list of strings that must appear) and `exit` (default
  0). A check that exits 0 for another reason fails, and the report says
  `expected output not observed`. Part of the criterion hash when present; a
  criterion without it hashes exactly as before.
- Stale statements (#261): identifiers named in a statement (backticks,
  `snake_case`, file names) that no longer appear in the code outside
  `.linebreak/` and Markdown produce a `stale_statements` finding.
- Shared tests (#263): two stories running the same test target (the `tests`
  payload or a runner's arguments inside a `command`, resolved against its
  `cd`) produce a `shared_tests` finding on each criterion involved, at
  `check` and at `spec approve`.
- New `criteria.integrity` in `gate.yml`: `warn` (default), `block` or `off`,
  for every kind or per kind (`shared_tests`, `stale_statements`,
  `zero_tests`). Under `block` the criterion fails (overridable like any
  failed check; `block_reasons` keep the published vocabulary). New
  `integrity` block in the JSON output (policy, findings, kinds skipped) and
  `tests_run` / `integrity` per criterion in the audit record.

Entorno medido, alcance por historia, liberación en dos fases y credenciales
del servicio de gobierno en un solo lugar (épica e10, hallazgos de la
construcción de Katun). Nada de esto cambia un bundle existente: los criterios
sin las claves nuevas conservan su hash y sus firmas.

- `check.resource: <slug>` (#262): los checks que nombran el mismo recurso
  compartido no se solapan en una misma máquina (lock del sistema en
  `$LINEBREAK_LOCK_DIR`, por defecto el directorio temporal; un lock retenido
  más allá del tiempo del check es error de herramienta y dice quién lo tiene).
  Un check exclusivo que falla se vuelve a correr una vez, solo, al final:
  si pasa queda `collision` (no bloquea, línea `collision:` y lista
  `collisions`, con la primera falla guardada); si falla otra vez queda `fail`
  con "reproduced when re-run alone". Documentado el patrón del inquilino
  desechable por corrida.
- `check.environment: {name, version_url, field}` (#264): el gate pregunta la
  versión desplegada antes y después del check y la registra por criterio con
  su estado frente al HEAD evaluado (`current`, `behind` con cuántos commits y
  cuánto atrás de main, `ahead`, `diverged`, `unknown`, `unreachable`,
  `changed`). Todo lo que no sea `current` es advertencia (`environment
warning:` y `environment_warnings`), nunca bloqueo. Queda en el registro de
  auditoría y `publish` lo pone al frente del detalle del criterio.
- Liberación en dos fases: `check.when: attestation` (solo en criterios
  `manual`) y `check --phase prepare|verify` (también `ci --phase` y la entrada
  `phase` de la Action). En `prepare` ese criterio sale `awaiting-attestation`
  (no se evalúa ni bloquea) y todo lo demás se exige, los otros `manual`
  incluidos; el registro dice `phase: prepare` y no es veredicto. `verify`
  (por defecto) lo exige como cualquier `manual`. Reemplaza el atajo de
  `--manual warn` que aflojaba todos los `manual`.
- `check --story auto` (#256): la historia de una rama `feat/<id>` o
  `story/<id>`, si no las historias iniciadas; la misma regla que ya tenían
  `ci` y la Action.
- `report --from-governance [--project] [--run-id] [--stage]`: el resumen de
  seguridad de la última corrida publicada en el servicio de gobierno, fuera de
  CI y en solo lectura (`GET /v1/reports/projects/{id}` y
  `GET /v1/projects/{id}/gate-runs/{run_id}`).
- Credenciales del servicio de gobierno en un solo lugar (`governance_env`):
  variables de entorno, luego `~/.config/linebreak/governance.env`, luego
  `governance-local.env`, luego `~/.linebreak/env` por compatibilidad; el
  primer archivo con token se usa entero. Lo usan `check`, `signoff`,
  `override`, los tickets, `publish` (ahora `--to` es opcional) y `report`.
  `LINEBREAK_GOV_CREDENTIALS=off` apaga los archivos. Cambio de conducta
  local: con un token en esos archivos, `signoff` y `override` registran la
  identidad verificada del servicio, igual que con la variable exportada.
- JSON de `check`: claves nuevas `phase`, `awaiting_attestation`,
  `collisions` y `environment_warnings` (siempre presentes, vacías cuando no
  aplican).

## 1.14.0

Signatures and configuration from the governance service (September wave,
`panel-gobierno` front). The panel signs; the gate verifies.

- `publish` now sends, with every run, the signed specification (`spec`:
  hash, who signed, when, kid, self-approval), a summary of
  `.linebreak/roles.yml` (`roles`) and, per criterion, its `statement` and
  content `hash`. An ingest without these fields is still accepted.
- `check` (and therefore `ci`) consults `GET /v1/projects/{id}/signoffs`
  when `LINEBREAK_GOVERNANCE_BASE_URL`, `LINEBREAK_GOVERNANCE_TOKEN` and
  `LINEBREAK_GOV_PROJECT` are set, and treats each sign-off recorded from the
  panel as `identity_source: governance` under the same rules as the ones in
  the repository (criterion hash, `roles.yml`, identity). A service that does
  not answer neither blocks nor approves: it is reported and the check goes
  on with the repository's sign-offs. New key `governance_signoffs` in the
  JSON output.
- Tickets: when `gate.yml` does not set the `tickets` block, the gate reads
  the tracker configuration from the service (`GET /v1/gate/tickets-config`,
  pipeline tokens only). `gate.yml` and environment variables win.
- A sign-off made from the panel only counts when the run was published by
  this version or later, because it binds to the criterion hash that
  `publish` now sends.
