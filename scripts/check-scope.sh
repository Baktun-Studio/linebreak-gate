...
#   INPUT_PHASE   verify | prepare | "" (empty = verify); prepare is the release
#                 run an attestation is made on: criteria marked
#                 `check.when: attestation` are not evaluated (only with stage release)
...
phase="${INPUT_PHASE:-verify}"
case "$phase" in
  verify) ;;
  prepare)
    if [ "$stage" != "release" ]; then
      note "invalid 'phase' input 'prepare' with stage '$stage' (prepare applies only to release); gate stays closed"
      exit 2
    fi
    ;;
  *)
    note "invalid 'phase' input '$phase' (expected verify or prepare); gate stays closed"
    exit 2
    ;;
esac
...
if [ "$phase" = "prepare" ]; then
  note "release phase: prepare (the run to attest, not the release; run phase verify after the sign-off)"
fi
