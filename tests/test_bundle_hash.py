...
from linebreak_gate.spec_bundle import bundle_hash, prospective_bundle_hash
...
def test_prospective_bundle_hash_equals_on_disk_after_write(tmp_path):
    """LIN-52: the hash computed from the sidecar stories BEFORE writing equals
    bundle_hash(load_bundle(root)) AFTER a real write — so a governance request
    bound to the prospective hash matches exactly what lands on disk."""
    spec = tmp_path / ".linebreak" / "spec"
    (spec / "stories").mkdir(parents=True)
    for s in STORIES:
        (spec / "stories" / f"{s['id']}.yml").write_text(
            spec_bundle.dump_story_yaml(s), encoding="utf-8"
        )
    ...
    on_disk = bundle_hash(spec_bundle.load_bundle(tmp_path))
    prospective = prospective_bundle_hash(copy.deepcopy(STORIES), "epics_and_stories")
    assert prospective == on_disk


def test_prospective_hash_is_story_order_independent():
    a = prospective_bundle_hash(copy.deepcopy(STORIES), "epics_and_stories")
    b = prospective_bundle_hash(list(reversed(copy.deepcopy(STORIES))), "epics_and_stories")
    assert a == b


def test_prospective_hash_changes_with_source_phase():
    a = prospective_bundle_hash(copy.deepcopy(STORIES), "epics_and_stories")
    b = prospective_bundle_hash(copy.deepcopy(STORIES), "prd")
    assert a != b
