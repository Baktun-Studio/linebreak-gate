...
def test_all_runs_the_full_bundle(shim, tmp_path):
    ...
    assert code == 0
    assert _check_call(calls) == f"check --path {tmp_path} --manual warn --stage release"
...
def test_explicit_story_id_is_passed_through(shim, tmp_path):
    ...
    assert _check_call(calls) == f"check --path {tmp_path} --manual warn --stage release --story S2"
...
def test_auto_infers_the_story_from_a_feat_branch(shim, tmp_path):
    ...
    assert _check_call(calls) == f"check --path {tmp_path} --manual warn --stage release --story S1"
...
def test_auto_falls_back_to_started_only_when_branch_is_not_a_story(shim, tmp_path):
    ...
    assert (
        _check_call(calls)
        == f"check --path {tmp_path} --manual warn --stage release --started-only"
    )
...
def test_manual_defaults_to_block_outside_pull_request(shim, tmp_path):
    ...
    assert _check_call(calls) == f"check --path {tmp_path} --manual block --stage release"
...
def test_manual_explicit_value_wins(shim, tmp_path):
    ...
    assert _check_call(calls).endswith("--manual block --stage release")
