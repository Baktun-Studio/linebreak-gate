...
- `check` evaluates the approved acceptance criteria (`.linebreak/spec/`,
  written by the desktop app on gate approval) against the working tree:
  `build`/`tests`/`command` run for real, `manual` requires a recorded
  sign-off. Exit 0 all satisfied (or no bundle — a clean no-op), 1 blocking
  (fail or needs-signoff), 2 tool/config/bundle error (fail closed). Writes
  `.linebreak/audit/criteria.json`. See
  [`docs/CRITERIA_ENFORCEMENT.md`](../../docs/CRITERIA_ENFORCEMENT.md).
...
- `spec list` prints the approved acceptance criteria bundle (see
  [`docs/SPEC_HANDOFF.md`](../../docs/SPEC_HANDOFF.md)): each story, its criteria
  with check types, and the approver attribution. Read-only. Exit 0 on a valid
  bundle _or when none exists_; exit 2 on a malformed bundle (fail closed on
  structure).
