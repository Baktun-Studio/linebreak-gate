...
- `spec list` prints the approved acceptance criteria the LineBreak desktop app
  hands off to the repo (`.linebreak/spec/`, written on gate approval — see
  [`docs/SPEC_HANDOFF.md`](../../docs/SPEC_HANDOFF.md)): each story, its criteria
  with check types, and the approver attribution. **Read-only and enforces
  nothing.** Exit 0 on a valid bundle _or when none exists_; exit 2 on a
  malformed bundle (fail closed on structure). It is the seam a future
  enforcement step would build on.
